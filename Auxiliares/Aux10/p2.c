...
Nodo *buscar(Nodo *a, char *val, char p) {
  ...
  else { // caso más de un proceso
    int fds[2];
    pipe(fds); // 0: lectura y 1: escritura
    int pid = fork();
    if (pid==0){ // proceso hijo
      Nodo *rizq=buscar(a->izq, val, p/2); // el procesos hijo resuelve el problema hacia la izquierda
      ...
      close(fds[1]);
      Nodo *rder=buscar(a->der, val, p/2); // el procesos padre resuelve el problema hacia la derecha
      Nodo *rizq;
