...
void suma_par(double x0, double dx, int n, double *pres) {
    double s= 0;
    int fds[n][2];
    for (int k= 0; k<n; k++) { // se crean n pipes
        if (pipe(fds[k]) == -1) {
            perror("pipe");
            exit(1);
        }
    ...
    pid_t pid = fork();
    for (int k= 0; k<n; k++) {
        if (pid == 0) { // el hijo se encarga de calcular g
            close(fds[k][0]);
            double x = g(x0+dx*k);
            ...
            s+= f(x); // el padre calcula f usando el calculo escrito por el hijo en el pipe
            close(fds[k][0]);
        }
    }
    if (pid == 0) { // el proceso hijo solo termina
        exit(0);
    }
    else {
        waitpid(pid, NULL, 0); // el proceso padre debe enterrar al hijo
    }
    *pres= s;
}
