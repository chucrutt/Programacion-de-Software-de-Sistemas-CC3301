...
.while_begin:           #     // del ciclo para mayor eficiencia
    ...
    # --- Contar bits en 1 de p[0], resultado en t2 ---
    mv      t2, zero        # t2 = contador
    mv      t3, a0          # copia de p[0]
.loop0:
    beq     t3, zero, .done0
...
.done0:
    ...
    # --- Contar bits en 1 de p[1], resultado en t3 ---
    mv      t3, zero
    mv      t5, a1
.loop1:
    beq     t5, zero, .done1
...
.done1:
    ...
    # Comparar t2 (bits de p[0]) con t3 (bits de p[1])
    li      t1, 0
    bge     t2, t3, .done_cmp
    li      t1, 1
.done_cmp:
